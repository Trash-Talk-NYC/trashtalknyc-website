...
The byline, the sign-off name, and the meta clause are composed in `src/lib/founder.ts`, which reads `name`/`role.en` from `getTeamMember('david')` rather than spelling David's name and title out, so a role rename propagates to the page (both languages) and to the meta without this page becoming a rename touchpoint.
