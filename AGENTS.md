...
## Our Story — /about/our-story, David's account VERBATIM
...
**The captain confirmed on 2026-08-13 that it is David's story**, settling the earlier ambiguity (the copy arrived from Nandi — "Source: Nandi Brooks email" — but reads as the founder in the first person), and both attributions were restored on that word: the on-page byline and the meta description's "told by …" clause.
...
His given title is "Trash Talk NYC's Story"; the nav/footer label is "Our Story" (ES `Nuestra Historia`).
The page opens with the site-standard `PageHero` (eyebrow "About · Our Story", ghost word "88 DAYS") carrying that title, split into two leaves so the toggle can restructure it — EN "Trash Talk NYC’s / Story" becomes ES "La historia de / Trash Talk NYC" leaf by leaf, because `setLang` swaps leaves and cannot reorder a heading.
...
The only presentation liberties, allowed as typesetting rather than addition: the lede sizing of the opening paragraph and typographic curly quotes where the source email had straight ones — including the `’` in the h1's "Trash Talk NYC’s", which must stay curly in both the rendered text and the `data-en` attribute the toggle restores.
...
Discovery is **deliberately** just the nav's About dropdown and the footer — the About page is not linked to the story and must not be without the captain's say-so: the About page was out of scope for this change, the captain asked for nothing added, and the story launches via an email linking straight to `/about/our-story`, so this is a deferral, not an oversight.
In `NavBar.astro`, the `!isStory` guard on `isTeam` is what stops "The Team" double-lighting on the story page while the About parent still does — the `/about/` prefix would otherwise match.
...
* Team identity (names, roles, bios, social-link slots, portrait crops, meta descriptions) lives in `src/lib/team.ts`, consumed by the About page and — through `getTeamMember('david')` — by the Our Story byline and meta description.
...
The Our Story page names David's role in its byline and meta description but reads both from `team.ts` at build time, so it is not a touchpoint (see "Our Story" above).
