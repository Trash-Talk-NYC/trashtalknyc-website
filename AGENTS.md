...
* **`viewport-fit=cover` is deliberately absent from the viewport meta, and re-adding it reintroduces an unfixable bug.**
With `cover`, scrolling page content renders into the status-bar/notch band; iOS 26 pins sticky/fixed elements to a layout viewport that lags collapsing browser chrome AND refuses to composite fixed/sticky layers into the region the chrome frees (Apple forums threads 800798, 801028) — so mid-scroll the header sat a full chrome-band below the visual top with raw page content visible above it, and no cover painted from the header's own layer could ever own that band.
Five attempts failed on the captain's device while passing every emulator and ≤iOS 18 simulator: chrome-color fixes (PRs #18, #22, #25), the out-of-box `nav::before` bleed, the 2026-08 in-box cover, and hide-on-scroll (the gap simply returned on scroll-up reveal, captain-confirmed 2026-08).
The working fix attacks the band's contents instead: without `cover`, iOS letterboxes the unsafe areas itself and page content never enters the band, so there is nothing to leak — the band is painted charcoal by the browser via the `theme-color` meta (iOS 15–17) or the `body` `background-color` (iOS 26 Liquid Glass ignores `theme-color` and samples `body` — not `html`, not the nav, not pseudo-elements; verified by pixel-sampling simulator screenshots).
...
Costs accepted with this trade: no edge-to-edge bleed under the notch in landscape (iOS pillarboxes those strips charcoal), and all `env(safe-area-inset-*)` resolve to 0 (every usage has a `max()` design-spacing fallback, so nothing moves).
The in-box charcoal cover above the nav logo (extra top padding pulled offscreen with a matching negative margin and negative sticky `top`; flow height stays `--nav-h` = 63px, so anchor math is unaffected) is retained as zero-cost insurance for pre-26 iOS rubber-band/chrome-transition frames — keep it inside the border box, never in an out-of-box pseudo-element.
...
This is a **UX feature** (more reading viewport on phones; the hide interaction was captain-approved on device) — it is **not** the safe-area gap fix and must not be sold as one; see the `viewport-fit` bullet above.
