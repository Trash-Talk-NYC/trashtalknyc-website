...
The orbit always shows all five fruits: a borough with no upcoming events gets a grayscale "No events" fruit (`data-empty`) whose card is a "no upcoming events in X — yet" note with a signup nudge, while the mobile carousel and its dots only include boroughs that have events.
