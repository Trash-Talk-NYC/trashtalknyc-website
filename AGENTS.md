...
That move kept a permanent `/contact/join` → `/recruit` 301, declared in `astro.config.mjs` (`redirects`, the single source of truth the Netlify adapter emits `_redirects` from — never hand-write that file); the From Our Founder renames below deliberately got no such rule because those URLs were never public.
...
The nav's About dropdown ships with three items: **The Team** (`/about`), **From Our Founder** (`/about/from-our-founder` — see its own section below), and **Open Roles** (`/recruit` — the recruitment page).
...
## From Our Founder — /about/from-our-founder, David's account VERBATIM

The founder page (`src/pages/about/from-our-founder.astro`) carries a first-person account of how Trash Talk NYC started, received via Nandi (2026-08-12); the canonical source text lived at `firstmate/data/our-story-page/copy.md`.
...
The page was **renamed twice pre-launch by the captain** (both 2026-08-13): "Our Story" (`/about/our-story`) → "Letter from Founder" (`/about/letter-from-founder`) → **"From Our Founder"** (ES `De Nuestro Fundador`) — clean renames with **no redirects**, deliberate because nothing had gone live, so neither old URL ever existed publicly; the author's original title "Trash Talk NYC's Story" lives in git history if it ever returns.
The page opens with the site-standard `PageHero` (eyebrow "About · From Our Founder", ghost word "88 DAYS") carrying the page name, split into two leaves so the toggle can restructure it — EN "From Our / Founder" becomes ES "De Nuestro / Fundador" leaf by leaf, because `setLang` swaps leaves and cannot reorder a heading.
...
The byline, the sign-off name, and the meta clause are composed in `src/lib/founder.ts`, which reads `name` and both `role` languages from `getTeamMember('david')` rather than spelling David's name and title out, so a role rename propagates to the page (both languages) and to the meta without this page becoming a rename touchpoint.
...
Discovery is **deliberately** just the nav's About dropdown and the footer — the About page is not linked to the story and must not be without the captain's say-so: the About page was out of scope for this change, the captain asked for nothing added, and the story launches via an email linking straight to `/about/from-our-founder`, so this is a deferral, not an oversight.
...
* Team identity (names, roles, bios, social-link slots, portrait crops, meta descriptions) lives in `src/lib/team.ts`, consumed by the About page and — through `getTeamMember('david')` in `src/lib/founder.ts` — by the From Our Founder byline and meta description.
...
The From Our Founder page names David's role in its byline and meta description but reads both from `team.ts` at build time, so it is not a touchpoint (see "From Our Founder" above).
