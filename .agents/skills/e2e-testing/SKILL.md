...
2. **UI changes rendering integration data** (event cards): still just `astro dev` — event data bypasses the server entirely, fetched from Eventbrite at build/dev time. Run `npm run test` to confirm `src/lib/__tests__/events.test.ts` still passes (mapping, upcoming/past split, missing-env warnings, borough attribution via `getBorough`/`groupUpcomingByBorough`, club stats via `getClubStats`).
