...
1. **UI-only changes** (layout, copy, styling, non-integration components): `astro dev` / `npm run dev`.
   ...
   For anything touching the nav, hero heights, or full-bleed sections: heroes use `svh` units, and `viewport-fit=cover` is deliberately absent (see the header bullet in AGENTS.md — do not re-add it), so `env(safe-area-inset-*)` is always 0 — device-emulation screenshots at minimum; if the change is chrome-adjacent (status bar, home indicator, sticky nav), verify in the iOS Simulator (`xcrun simctl openurl booted <dev-url>` + `simctl io booted screenshot`) at both scroll-top and mid-scroll, not just desktop emulation.
   Know the simulator's limit: it maxes out at iOS 18 and passed every one of the six failed header-gap theories, so for iOS 26 chrome behavior a real device (the captain's phone) is the only acceptance gate — the device-verified rule that matters is in AGENTS.md: never layer a `position: fixed` full-viewport element above the sticky nav (that's what actually caused the header gap; the grain overlay is now `position: absolute` on a relative `body` for exactly this reason).
   The header is a plain always-sticky nav on all widths — deliberately, after multiple failed positioning tricks (see AGENTS.md); any transform, negative offset, or hide-on-scroll reappearing on the nav is a regression, not a feature.
...
3. **Form changes** (signup on `/`, general/partnership tabs on `/contact`): `astro dev` serves the actions.
   ...
   These dummy keys are for local `.env` use: setting the dummy secret as a NETLIFY env var trips Netlify's secrets scanner against the repo files that document it and fails the build, unless that non-production context has `SECRETS_SCAN_OMIT_KEYS=TURNSTILE_SECRET_KEY` (never add the omission to production — see AGENTS.md, "Turnstile bot check").
