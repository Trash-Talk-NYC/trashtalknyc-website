...
4. **Server-logic changes** (schemas, spam heuristics, rate limiting, Turnstile verification, Brevo client, GoFundMe fetch/parse in `src/lib/server/`): these are plain modules with unit tests in `src/lib/server/__tests__/` — extend those first; they run without any server.
...
6. **Wallet page / donation total** (`/wallet`, `/api/donation-total.json`, `src/lib/server/gofundme.ts`): `astro dev` serves both the build-time bake-in and the on-demand API route. Confirm the page renders the build-time total, then confirm `/api/donation-total.json` returns `{ amountRaised }` and the page's count-up picks it up client-side (the live number is deliberately held back ~1.1s until the cards' entrance animation finishes). Exercise the 3D wallet's card slot: hover (pointer devices) or keyboard `:focus-visible` (any device) slides a card up out of the slot; on touch (`hover: none`) left/right arrow buttons flank the wallet — pressing one swipes the front card out sideways and behind the stack while the other card rises to the front (check the arrows are hidden on hover devices, and that tapping a card always navigates). Check the `prefers-reduced-motion` path (entrance and swipe animations are disabled — an arrow press swaps the cards instantly — and count-up should skip straight to the final value) and confirm a GoFundMe outage degrades to the `$—` fallback rather than failing the page. `/wallet` is linked from nav as "The Wallet" / "La Cartera" (desktop nav and mobile hamburger), between Club Events and About.
