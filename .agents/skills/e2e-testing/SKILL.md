...
1. **UI-only changes** (layout, copy, styling, non-integration components): `astro dev` / `npm run dev`.
   ...
   The mobile header (≤768px) is hide-on-scroll by design: scrolling down hides it, any upward scroll reveals it, and it is always shown at the top — a hidden header mid-scroll on mobile is expected, not a regression. For nav-touching changes, verify the hide/reveal cycle, that the header never hides while the hamburger menu is open, and that crossing the 768px breakpoint never strands a hidden header (logic in `src/lib/headerScroll.ts`, wiring and guards in `NavBar.astro` — see AGENTS.md).
   For in-page anchor links (`#deep-clean`, `#signup`), confirm the target lands clear of the sticky nav — `scroll-padding-top` on the `html` rule in `global.css` (derived from `--nav-h`) handles this, so a regression there usually means that token drifted from the nav's real height. It is deliberately sized for the *shown* header (which also covers the hidden state), so anchors land clear regardless of hide-on-scroll.
