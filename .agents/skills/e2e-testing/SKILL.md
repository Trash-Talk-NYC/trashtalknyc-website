...
6. **Wallet page / donation total** (`/wallet`, `/api/donation-total.json`, `src/lib/server/gofundme.ts`): `astro dev` serves both the build-time bake-in and the on-demand API route. Confirm the page renders the build-time total, then confirm `/api/donation-total.json` returns `{ amountRaised }` and the page's count-up picks it up client-side (the live number is deliberately held back ~1.1s until the cards' entrance animation finishes). Exercise the 3D wallet's card slot: hover or `:focus-visible` slides a card up out of the slot on pointer devices; on touch (`hover: none`) the first tap pops the card, the second tap navigates, and tapping elsewhere settles it back. Check the `prefers-reduced-motion` path (entrance animation and pop transitions are disabled; count-up should skip straight to the final value) and confirm a GoFundMe outage degrades to the `$—` fallback rather than failing the page. `/wallet` is linked from nav as "The Wallet" / "La Cartera" (desktop nav and mobile hamburger), between Club Events and About.
