...
6. **Clean Zone map changes** (`/clean-zone`, `CleanZoneMap.astro`, `src/lib/clean-zone.ts`): `astro dev` — the map is client-only (Leaflet + OSM tiles, no build-time fetch or Astro Action). Run `npm run test` for the `zoneCircle` geometry unit tests, then verify in the browser: every `cleanZones` entry renders a shaded zone (circle from `zoneCircle`, or its hand-traced `outline`) with a permanent name tag, plus a small dot per tree bed inside it; the zone popup lists its beds and each bed dot opens a language-neutral mini popup (address + optional NYC tree ids); switching EN/ES re-renders zone tooltips and popups (`onLanguageChange`, since Leaflet's popup/tooltip DOM can't use `data-en`/`data-es`) while bed-dot popups stay language-neutral; a zone log card's "Find it on the map" button focuses its zone and opens its popup (`clean-zone:focus` CustomEvent); the header sticker's zone/bed counts match the data; on a touch emulation, one-finger drag is disabled until the first zoom gesture, then works; scroll-wheel over the map scrolls the page, not the map.
