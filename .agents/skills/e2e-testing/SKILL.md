...
6. **Clean Zone map changes** (`/clean-zone`, `CleanZoneMap.astro`, `src/lib/clean-zone.ts`): `astro dev` — the map is client-only (Leaflet + OSM tiles, no build-time fetch or Astro Action). Verify: markers/popups render for every `guardedSites` entry; switching EN/ES re-renders open and future popups (`onLanguageChange`, since Leaflet's popup DOM can't use `data-en`/`data-es`); a field-log card focuses its marker and opens the popup (`clean-zone:focus` CustomEvent); on a touch emulation, one-finger drag is disabled until the first zoom gesture, then works; scroll-wheel over the map scrolls the page, not the map.
